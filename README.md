...
| | |
|---|---|
| 번역 완료 | **996 / 1,230** 세그먼트 (81.0%) |
| 바이트 기준 | 53,728 / 58,113 (**92.5%**) |
| 고유 음절 | 798 / **1,314** |
| 최종 인벤토리 외삽 | 약 836자 (여유 478자) |
| 예산 초과 | 0개 |
| 삽입 역검증 | 8단 전부 통과 |
...
| | |
|---|---|
| 번역 완료 | **32 / 179** 조각 (17.9%) |
| 바이트 기준 | 462 / 7,294 (6.3%) |

엔딩은 대사와 다른 렌더러(`0x005666`)를 쓰고 폰트 영역 안(`0x2FCFC0`~`0x2FF780`)에
있습니다. 포인터 표가 4개뿐이라 재배치도 가능하지만 제자리로 했습니다 — 표를
건드리지 않으면 참조 문제가 없습니다.

조각은 줄바꿈(`0xF1`) 단위로 끊기고 원문은 앞 공백으로 가운데맞춤이 돼 있습니다.
**이 공백이 예산입니다.** 조각당 8~17바이트라 여유가 거의 없어, 넘칠 때는 공백을
줄이거나 2바이트 음절을 피해 고쳐 씁니다. `endbatch.py cost`가 조각별 비용과 어떤
음절이 2바이트인지 보여줍니다.

**엔딩은 강제 배정 반복에서 제외합니다.** 넣었더니 대사 예산 초과가 2개에서 72개로
뛰었습니다. 엔딩 조각이 1바이트 넘치면 그 조각 음절 열 개가 전부 단일바이트 칸을
받는데, 1바이트를 아끼려 칸 열 개를 쓰는 거래입니다. 밀려난 고빈도 음절은 대사
1,000개에 걸쳐 비용을 냅니다. 단어표·이름표는 칸이 고정(`腕輪`은 2바이트)이라
강제가 유일한 수단이지만, 엔딩은 고쳐 쓸 여지가 넓습니다.

제외 후: 강제 배정 80자 → 32자, 단일바이트 커버리지 76% → 79%,
평균 1.24 → 1.21 바이트/음절.
