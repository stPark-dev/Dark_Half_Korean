...
1. **몬스터 이름표** (`0x04f672~`) — **구조는 풀렸습니다**(107엔트리,
   `montbl.py`). 막는 것은 구조가 아니라 칸입니다. 타일 직접 렌더링이라면
   부품 55개를 옮기는 데 새 음절 **41자**가 필요한데 **6칸**뿐이고, 표 안의
   단발 음절을 전부 되팔아도 30칸입니다. 먼저 실기 한 장으로 렌더러를
   확정해야 합니다 (PROGRESS 4.39).
2. **「サウンド」 계열 3항목** (`0x2FF734~`) — 번역은 넣었지만 그 항목을 그리는
   화면을 못 찾아 실기 확인을 못 했습니다.
3. **실기 통과 플레이** — 화면별 확인은 했지만 처음부터 끝까지 돌려 본 적이
   없습니다.
