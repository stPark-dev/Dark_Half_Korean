...
| 아이템 창 (전) | 설명문은 정상 | 마왕 첫 메뉴 (전) |
|---|---|---|
| ![아이템 이름 깨짐](image/개선.png) | ![설명문 한글](image/개선2.png) | ![마왕 메뉴](image/개선안됨_마왕초기.png) |
...
**마법 목록의 반각 폰트를 찾았습니다.** 압축 그래픽 **엔트리 0**의 블록 0~3이
8×8 2bpp 반각 폰트이고, VRAM `0x2000`부터 **문자 코드순**으로 올라갑니다
(`0x00~0x3F` / `0x40~0x7F` / `0x80~0xBF` / `0xC0~0xFF`). 타일 뷰어 스크린샷을
VRAM 덤프와 정렬해 문자 베이스 512를 구했습니다.

그리고 목록은 **바이트 하나 = 타일 하나**로 그립니다 — 타일맵의 타일 번호가
대사 폰트의 단일바이트 코드와 그대로 같습니다(`8C 6B B0 92 B0 7C 6E 9D` =
`フォーメーション`). 이스케이프를 해석하지 않으므로 이름 음절은 전부
단일바이트여야 합니다.

한글을 넣으려니 원본을 그대로 재압축해도 칸을 넘어서, **packer에 역참조(복사)
명령을 구현**했습니다. 원본 재압축이 896→745, 969→688, 1061→770, 902→813으로
내려가 원본 크기와 거의 같아졌고, **1299/1299 블록 왕복 검증**을 통과했습니다.
그 여유로 단일바이트 142칸에 8×8 한글을 넣어도 네 블록 전부 들어갑니다.
