...
`v0.9.3` 입니다. 대사·UI·메뉴·오프닝·컷신·아이템/장비/마법 이름은 끝났고,
...
| 이름표 (몬스터 · 소환 목록) | 10% | 40% | 미번역 30개 + 목록용 18음절 단일바이트화 |
| 컷신 「復活節第１日」 | 2% | 100% | — (26줄, **실기 확인**) |
| 실기 전체 검증 | 8% | 30% | 통과 플레이 |
| **가중 합** | | **≈ 88%** | |

버전 번호는 백분율이 아니라 **이정표**입니다. 가중 합은 88%지만 남은 일이
...
| 검사 | 텍스트 **12층** + 삽입 역검증 **19단** + `test_kr` 10군 전부 통과 |
...
## 최근 작업 — 실기 다섯 장: 셋은 고치고 둘은 원본이었습니다
...
번역문 검사 12층(`trcheck.py`)과 삽입 후 역검증 19단(`verify_insert.py`)입니다.
