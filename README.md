...
`v0.9.0` 입니다. 대사·UI·메뉴·오프닝·아이템/장비/마법 이름은 끝났고, 남은 것은
몬스터 이름표와 컷신 한 종, 그리고 전체 플레이 검증입니다.
...
| 컷신 「復活節第１日」 | 2% | 0% | 그 화면의 VRAM 덤프가 필요 |
| 실기 전체 검증 | 8% | 30% | 통과 플레이 |
| **가중 합** | | **≈ 86%** | |

버전 번호는 백분율이 아니라 **이정표**입니다. 가중 합은 86%지만 남은 일이
「표 하나 + 컷신 하나 + 검증」으로 좁아졌으므로 `v0.9.0`으로 잡았습니다.
...
| 검사 | 텍스트 **12층** + 삽입 역검증 **17단** + `test_kr` 10군 전부 통과 |
...
2. **컷신 「復活節第１日 / わがうちに憂いは満ちぬ」** — 못 찾았습니다. 대사
   인코딩으로도 오프닝식 스프라이트 스크립트로도 안 나옵니다. 그 화면의 VRAM
   덤프가 있으면 오프닝과 같은 순서로 풀립니다 (PROGRESS 4.28).
3. **「サウンド」 계열 3항목** (`0x2FF734~`) — 번역은 넣었지만 그 항목을 그리는
   화면을 못 찾아 실기 확인을 못 했습니다.
4. **실기 통과 플레이** — 화면별 확인은 했지만 처음부터 끝까지 돌려 본 적이
   없습니다.
...
번역문 검사 12층(`trcheck.py`)과 삽입 후 역검증 17단(`verify_insert.py`)입니다.
...
**컷신 「復活節第１日 / わがうちに憂いは満ちぬ」** — 못 찾았습니다. 대사
인코딩으로 검색해도(「わがうちに」 = `dc e6 b3 c1 c6`) 없고, 満·復·第 는
단일바이트 표에 없으며, 오프닝식 스프라이트 스크립트도 `0x90000~0xA0000` 을
전수 훑어 오프닝 것 하나뿐입니다. 그 화면의 VRAM 덤프가 필요합니다.
