...
| 사용 중 | 793자 (여유 459자) · 쌍둥이 6칸 · 평균 1.24 바이트/음절 |
...
| 목록 창 (아이템·장비·마법) | **98엔트리 전부 단일바이트 한글** — 반각 8×8 폰트 141자 |
| 롬 | 3MB → **4MB** (엔진 패치 기본 적용) |
| 검사 | 텍스트 **12층** + 삽입 역검증 **16단** + `test_kr` 10군 전부 통과 |
...
번역문 검사 12층(`trcheck.py`)과 삽입 후 역검증 16단(`verify_insert.py`)입니다.
