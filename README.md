...
| 검사 | 텍스트 **11층** + 삽입 역검증 **11단** + `test_kr` 10군 전부 통과 |
...
번역문 검사 11층(`trcheck.py`)과 삽입 후 역검증 11단(`verify_insert.py`)입니다.
...
**오프닝 컷신 — 글리프는 찾았고 타일맵이 남았습니다.** 텍스트가 아니라
**압축 그래픽**이었습니다. 엔트리 18의 블록 `0x159197`·`0x159321`에 내레이션
전체의 글리프가 **중복을 뺀 첫 등장 순서**로 들어 있습니다(88칸,
`image/오프닝글리프.png`). 스크린샷에서 글리프를 직접 뽑아 템플릿으로 쓰니
15자 전부 IoU 1.000으로 일치했습니다.
...
남은 것은 **조합용 타일맵**입니다. 한국어는 어순이 달라 글리프만 바꿔선 안
됩니다. 인덱스 바이트열·워드·간격 1~8·부분수열·타일맵 서명·복호 블록 내부를
모두 찾았지만 없습니다(차이 수열로 찾았으므로 기준값과 무관하게 없음).
**오프닝 텍스트가 떠 있는 순간의 VRAM·타일맵 덤프가 필요합니다.**
