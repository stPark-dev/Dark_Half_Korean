...
| 고유 음절 수용량 | 811자 → **1,257자** (엔진 패치, `F4` 제외) |
| 사용 중 | 804자 (여유 453자) · 쌍둥이 6칸 · 평균 1.21 바이트/음절 |
...
| 검사 | 텍스트 **12층** + 삽입 역검증 **12단** + `test_kr` 10군 전부 통과 |
...
**그런데 마법 목록 창은 다릅니다.** 그 창의 「フォーメーション」이 깨끗한 일본어로
나오는데, 그 바이트 여덟 개는 전부 한글로 회수돼 있습니다(フ→를, ォ→용, ー→말…).
패치된 대사 폰트로 그리면 한글이 나와야 하므로 **이 창은 그 폰트를 쓰지 않습니다.**
그 폰트는 아직 못 찾았습니다 — 롬 평문에 사본이 없고(글리프마다 1곳, 전부 패치됨),
41엔트리 압축 그래픽에도 없습니다(최고 IoU 0.545).

화살표를 움직이면 **화면 전체가 붕괴**합니다. 뱅크 프리픽스를 모르는 렌더러가
`f5`를 글리프 번호로 읽고 폭주하는 것으로 보이지만 확정하지 못했습니다.
...
번역문 검사 12층(`trcheck.py`)과 삽입 후 역검증 12단(`verify_insert.py`)입니다.
