...
`v0.9.5` 입니다. 대사·UI·메뉴·오프닝·컷신·아이템/장비/마법 이름은 끝났고,
남은 것은 몬스터 이름표와 전체 플레이 검증입니다.
...
| 이름표 (몬스터 · 화자) | 10% | 70% | 107엔트리 중 55개 (B안). 나머지는 A안이 필요 |
| 컷신 「復活節第１日」 | 2% | 100% | — (26줄, **실기 확인**) |
| 실기 전체 검증 | 8% | 30% | 통과 플레이 |
| **가중 합** | | **≈ 91%** | |

버전 번호는 백분율이 아니라 **이정표**입니다. 가중 합은 91%지만 남은 일이
「표 하나 + 검증」으로 좁아졌으므로 `v0.9.x`로 잡았습니다.
