...
| 슬롯 | 개수 | 비고 |
|---|---|---|
| 단일바이트 | 145 | `0x20-0xDF` 중 보존 코드 47개 제외 |
...
원래 엔진은 글리프 인덱스가 10비트여서 1,024자가 절대 상한이었습니다.
전량 번역에는 약 1,200자가 필요해 렌더러를 패치했습니다.

핵심은 발견한 성질 하나입니다. 인덱스 1024를 `ASL×6`(×64)하면 `0x10000`이
되어 **16비트에서 하위 `0x0000`으로 자연히 감깁니다.** 즉 하위 16비트는 이미
...
| 주소 | 변경 | 크기 |
|---|---|---|
| `0x00950F` | `JMP $F200` — 이스케이프 판별 확장 (메인 대사) | 39바이트 |
| `0x005CFA` | `JMP $F260` — 이스케이프 판별 확장 (옵션·엔딩) | 37바이트 |
| `0x009548` | `JMP $F300` — DMA 꼬리 재구성 | 63바이트 |
| 롬 크기 | 3MB → 4MB (뱅크 `$F0` = ROM `0x300000`) | |

프리픽스로는 `$5D`(備)·`$D5`(ゆ)를 씁니다. 이 두 바이트는 우리가 번역하지 않는
구간에서 한 번도 쓰이지 않습니다. 뱅크 계산이 `$6A >> 2`이므로 `$6A`=6,7
(인덱스 1536~2047)도 **코드 변경 없이** 쓸 수 있어 확장 여지가 남아 있습니다.

부작용: 이 두 바이트를 단일바이트 글리프로 배정할 수 없어 단일바이트 슬롯이
145→143으로 줄었고, 아직 번역하지 않은 일본어 중 `備`/`ゆ`가 든 것은 깨져
보입니다 (전량 번역 시 해소).
