...
1. **몬스터 이름표** (`0x04f672~`) — 30개 미번역. 포인터 표 `0x04f587` 의
   구분자가 `0xFF` 가 아니고 엔트리가 겹쳐 읽힙니다. 번역된 19개도 마왕 소환
   목록에서는 깨집니다 — 18음절이 2바이트여서입니다(디즈배트크킹코볼운프웨타
   뉴몬골렘래곤). 목록용 단일바이트 칸은 110개 중 104개가 이미 차 있어
   **이름을 다시 짓는 작업이 같이 필요합니다**(해골·골렘 등).
...
**몬스터 이름표** — 포인터 표 `0x04f587` 의 구분자가 `0xFF` 가 아니고 엔트리가
겹쳐 읽힙니다. 미식별로 남긴 `F7` 뱅크 상위 약 55개 코드가 대사가 아니라
가타카나 몬스터 이름 안에 나타납니다(`ヘル<C79>ルベロス` 등) — 이 구간은 `0xF7`
을 뱅크 이스케이프로 쓰지 않습니다. 아이템 표와 같은 접미어 공유일 가능성이
크고(「オーク」「オークキング」「オークロード」가 이어져 있습니다), 조립기
흉내(역검증 [16])를 이 표에도 붙이면 경계 규칙이 드러날 것으로 봅니다.
